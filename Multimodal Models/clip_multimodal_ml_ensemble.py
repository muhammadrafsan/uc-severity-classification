# -*- coding: utf-8 -*-
"""CLIP multimodal ensemble without text feature.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1G2_70rQNL_2HBU1n1D73RTIjpTlukJga
"""
...
mild_images = [os.path.join(mild_images_dir, img) for img in os.listdir(mild_images_dir) if img.endswith(('.png', '.jpg'))]
severe_images = [os.path.join(severe_images_dir, img) for img in os.listdir(severe_images_dir) if img.endswith(('.png', '.jpg'))]
...
total_end_time_1 = time.time()
print(f"\nTOTAL PROCESSING TIME (FEATURE ENCODING + MODEL TRAINING): {total_end_time_1 - total_start_time:.2f} seconds")
...
print(f"\nTOTAL PROCESSING TIME (FEATURE ENCODING + MODEL TRAINING): {total_end_time_2 - total_start_time:.2f} seconds")
...
single_sample = test_features[0:1]
start_time = time.time()
_ = voting_clf.predict(single_sample)
print(f"\nSingle image prediction time: {(time.time() - start_time)*1000:.2f} ms")


ten_samples = test_features[:10]
start_time = time.time()
_ = voting_clf.predict(ten_samples)
print(f"10 images prediction time: {(time.time() - start_time)*1000:.2f} ms")


start_time = time.time()
predicted_labels = voting_clf.predict(test_features)  # Existing line
print(f"Full test set prediction time: {time.time() - start_time:.2f} sec")

print("\nStacking Classifier Prediction Times:")


single_sample = test_features[0:1]
start_time = time.time()
_ = stacking_clf.predict(single_sample)
print(f"Single image prediction time: {(time.time() - start_time)*1000:.2f} ms")


ten_samples = test_features[:10]
start_time = time.time()
_ = stacking_clf.predict(ten_samples)
print(f"10 images prediction time: {(time.time() - start_time)*1000:.2f} ms")

start_time = time.time()
stacking_predicted_labels = stacking_clf.predict(test_features)
print(f"Full test set prediction time: {time.time() - start_time:.2f} sec")
